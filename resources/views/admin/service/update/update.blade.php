<div class="space-y-6 font-sans">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center justify-between pb-4 border-b border-[#F3E9DC]/60">
        <div>
            <h2 class="text-lg font-bold text-brownie tracking-tight uppercase">Edit Service</h2>
            <p class="text-xs text-brownie/60 font-medium mt-0.5">Update service details, cover image, and rich text content via TinyMCE editor.</p>
        </div>
    ...
    <!-- Edit Service Form Container Card -->
    <div 
        x-data="{
            content: @entangle('content'),
            initTinyMCE() {
                if (typeof tinymce === 'undefined') return;
                tinymce.remove('#service-update-content-editor');
                tinymce.init({
            ...
            <div class="space-y-1">
                ...
                <div class="flex items-center gap-4">
                    ...
                    @elseif($existingImage)
                        <img src="{{ $existingImage }}" class="w-14 h-14 rounded-xs object-cover border border-[#F3E9DC]" />
                    @endif
                    <input 
                        wire:model="image" 
                        type="file" 
